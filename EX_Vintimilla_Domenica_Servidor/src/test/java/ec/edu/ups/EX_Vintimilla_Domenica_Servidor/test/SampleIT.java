package ec.edu.ups.EX_Vintimilla_Domenica_Servidor.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/EX_Vintimilla_Domenica_Servidor.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "EX_Vintimilla_Domenica_Servidor.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("ec.edu.ups.EX_Vintimilla_Domenica_Servidor.test");
